"""
Ablation: J3/I60 — INIT ONLY (no RKD)
=======================================
AGT initialisation from teacher's last 3 inner steps.
Task loss only — no RKD supervision.
"""
...
N_INNER_T     = n_iter_inner_J20        # 20
N_INNER_S     = 3                       # J3 student
INNER_START_T = N_INNER_T - N_INNER_S  # = 17
...
def build_last3_init(teacher_model, n_outer_S=60):
    """
    Initialise student step_size [3, 60, K+1] from teacher's last 3
    inner steps [17, 18, 19].

    Step 1 — extract teacher inner steps [17:20] → [3, 120, K+1]
    Step 2 — average over outer iterations → [3, 1, K+1] fingerprint
    Step 3 — broadcast uniformly to [3, 60, K+1]
    """
    with torch.no_grad():
        ss_T        = teacher_model.step_size.data       # [20, 120, K+1]
        ss_last3    = ss_T[INNER_START_T:].clone()       # [3, 120, K+1]
        fingerprint = ss_last3.mean(dim=1, keepdim=True) # [3,  1, K+1]
        ss_init     = fingerprint.expand(
                          -1, n_outer_S, -1).clone()     # [3, 60, K+1]

    print(f"\n[J3 LAST-3 init — NO RKD]")
...
if run_RKD_Distillation == 1:
    ...
    print('=' * 70)
    print('INIT ONLY (no RKD)  J3/I60  ←  J20/I120')
    ...
    # Load teacher — needed for init only
    ...
    # Build AGT init from teacher last-3 inner steps
    ss_init = build_last3_init(model_teacher, n_outer_S=I_S)

    model_student = PGA_Unfold_J3_I60_CI(ss_init).to(device)
    ...
    for i_epoch in range(n_epoch):
        ...
        print(f"Epoch {i_epoch:4d} [J3/I60 | init_only] | "
        ...
        if current_student > best_student_loss:
            ...
            torch.save(model_student.state_dict(),
                       ...
                       '_J3_I60_INIT_ONLY_best.pth')
            print(f"  [BEST] epoch {i_epoch} → saved")

    torch.save(model_student.state_dict(),
               model_file_name_UPGA_J10 + '_J3_I60_INIT_ONLY.pth')
    print(f'\nSaved.')
...
if run_RKD_Distillation == 0:
    ...
    model_test = PGA_Unfold_J3_I60_CI(0.01).to(device)
    model_test.load_state_dict(torch.load(
        model_file_name_UPGA_J10 + '_J3_I60_INIT_ONLY_best.pth',
        map_location=device))
    model_test.eval()
