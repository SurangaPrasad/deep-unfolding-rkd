...
# ══════════════════════════════════════════════════════════════════════════════
I_T =60          # teacher outer iterations
I_S =30          # student outer iterations
...
# CHOOSE CELL
# ══════════════════════════════════════════════════════════════════════════════
ABLATION_CELL = 'cell_2'   # 'cell_1'|'cell_2'|'cell_3'|'cell_4'
# ══════════════════════════════════════════════════════════════════════════════
...
def build_agt_init(teacher_model):
    ...
    """
    with torch.no_grad():
        ss_T = teacher_model.step_size.data   # [20, 80, K+1]fsave
...
step_size_seed = 0.01
K_layers       = 15              # window length (shorter since I_S=20)
...
model_teacher.load_state_dict(
    torch.load('UPGA_J20_I60_w_0_30.pth', map_location=device))
model_teacher.eval()
for p_param in model_teacher.parameters():
    p_param.requires_grad_(False)
print('Teacher loaded: UPGA_J20_I60_w_0_30.pth\n')
...
save_path = (model_file_name_UPGA_J10 +
             ...
             f'_Kl{K_layers}_win{t_start_early}_030_prop.pth')
