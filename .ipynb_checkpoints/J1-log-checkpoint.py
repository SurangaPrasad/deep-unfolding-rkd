...
def build_agt_init_J1(teacher_model):
    """
    AGT for J_S=1:
      Stage 1: average all J_T=20 inner steps into a single step
      Stage 2: average over all I_T=120 outer iterations -> fingerprint
      Stage 3: broadcast to all I_S=120 student outer slots
    """
    with torch.no_grad():
        ...
        # Stage 1: collapse inner J_T -> J_S=1 by mean over all 20 inner steps
        ss_compressed = ss_T.mean(dim=0, keepdim=True)   # [1, 120, K+1]

        # Stage 2: collapse outer -> fingerprint [1, 1, K+1]
        fingerprint = ss_compressed.mean(dim=1, keepdim=True)

        # Stage 3: broadcast to [1, I_S, K+1]
...
save_path = (model_file_name_UPGA_J1 +
             ...
             f'_Kl{K_layers}_win{t_start_early}_0.15.pth')
