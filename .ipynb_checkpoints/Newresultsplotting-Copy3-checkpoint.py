...
# ── Plot-depth overrides ────────────────────────────────────────────────────
# Teacher is trained at I=120 but should only be *plotted/evaluated* up to I=80.
# Students are trained at I=60 but should only be *plotted/evaluated* up to I=40.
TEACHER_PLOT_I = 80
STUDENT_PLOT_I = 40
n_iter_outer=120
iter_number_UPGA_J20       = np.array(list(range(n_iter_outer + 1)))     # x-axis for conv / upga_j1 (unchanged, still full depth)
iter_number_teacher_trunc  = np.array(list(range(TEACHER_PLOT_I + 1)))   # x-axis for the teacher curve only
iter_number_UPGA_J10_I60   = np.array(list(range(STUDENT_PLOT_I + 1)))   # x-axis for all student curves (now truncated)
...
# ── Teacher (J=20, I=120), evaluated/plotted only up to I=80 ─────────────────
model_UPGA_J20 = PGA_Unfold_J20(step_size_UPGA_J20)
model_UPGA_J20.load_state_dict(
    torch.load('UPGA_J20_I120_w_030.pth', map_location='cpu'))
model_UPGA_J20.eval()
with torch.no_grad():
    sum_rate_J20, tau_J20, _, _ = model_UPGA_J20.execute_PGA(
        H_test, R, snr, TEACHER_PLOT_I, n_iter_inner_J20)
...
rate_iter_v2c1 = [r.detach().numpy()
                   for r in (sum(sum_rate_v2c1) / len(H_test[0]))][:STUDENT_PLOT_I+1]
tau_iter_v2c1  = [e.detach().numpy()
                   for e in (sum(tau_v2c1)      / len(H_test[0]))][:STUDENT_PLOT_I+1]
...
with torch.no_grad():
    sum_rate_v2c2, tau_v2c2, _, _ = model_v2_c2.execute_PGA(
        H_test, R, snr, STUDENT_PLOT_I, n_iter_inner_J10)
...
print(f'  {"-"*78}')
results_table = {
    f'Teacher (J=20, I=120, shown to I={TEACHER_PLOT_I})' : (obj_iter_J20[-1],    rate_iter_J20[-1],    tau_iter_J20[-1]),
    ...
    f'Student (J=10, I=60, shown to I={STUDENT_PLOT_I})' : (obj_iter_v2c1[-1],   rate_iter_v2c1[-1],   tau_iter_v2c1[-1]),
...
def conv_plot(fig_num, data_dict, ylabel, filename):
    plt.figure(fig_num)
    plt.plot(iter_number_teacher_trunc, data_dict['J20'],  '-',              color=COLORS['teacher'],    linewidth=3,   marker='o', markevery=8, markersize=6, label=LABELS['teacher'])
...
for ss in range(len(snr_dB_list)):
    ...
    with torch.no_grad():
        # Teacher, truncated to I=80
        _, _, F, W = model_UPGA_J20.execute_PGA(
            H_test, R_ss, snr_ss, TEACHER_PLOT_I, n_iter_inner_J20)
