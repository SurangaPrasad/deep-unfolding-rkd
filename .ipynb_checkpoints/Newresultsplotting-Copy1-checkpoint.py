...
# ── Teacher J20/I80 ───────────────────────────────────────────────────────────
_ckpt_J20_I80  = torch.load('UPGA_J20_I60_w_0_30_prop.pth', map_location='cpu',
                             weights_only=False)
_ss_shape_I80  = _ckpt_J20_I80['step_size'].shape   # [20, 80, 2]
...
_actual_I_J20_I80 = _ss_shape_I80[1]   # 80
with torch.no_grad():
...
# ── New student cell_1: flat init, task loss only ─────────────────────────────
print('Loading new cell_1...')
model_nc1 = load_new_student(
    model_file_name_UPGA_J10 +
    f'_IS{I_S_new}_IT{I_T_new}_cell_1_flat0.01_noRKD'
...
# ── New student cell_2: flat init + L_log + CI-RKD ───────────────────────────
...
# ── New student cell_3: AGT init, task loss only ──────────────────────────────
...
# ── New student cell_4: AGT init + L_log + CI-RKD ────────────────────────────
...
print('='*78)
print(f'  {"Model":<50} {"Obj":>8} {"R":>10} {"Tau":>10}  {"Steps":>8}')
print(f'  {"-"*80}')
results_table = {
    'Teacher (J=20, I=80)'               : (obj_iter_J20_I80[-1],  rate_iter_J20_I80[-1],  tau_iter_J20_I80[-1],  1600),
    f'Student J10/I{I_S_new}, flat'      : (obj_iter_nc1[-1],      rate_iter_nc1[-1],      tau_iter_nc1[-1],      J_S_new*I_S_new),
...
for name, (obj, rate, tau, steps) in results_table.items():
    print(f'  {name:<50} {obj:>8.4f} {rate:>10.4f} {tau:>10.4f}  {steps:>8}')
print('='*78 + '\n')
...
def conv_plot(fig_num, data_dict, ylabel, filename,
    ...
    ax.plot(iter_number_new_student, data_dict['nc1'],       ':',
            ...
            marker='h', markevery=4, markersize=6,
            label=LABELS['nc1'])
    ...
    if zoom_y is not None:
        ...
        axins.plot(iter_number_new_student, data_dict['nc1'],       ':',
                   ...
                   marker='h', markevery=4, markersize=3)
...
conv_plot(1, data_obj,
          r'$R - \lambda\bar{\tau}$',
          'new_obj_vs_iter',
          zoom_x=(20, 42), zoom_y=(13, 15.5))

conv_plot(2, data_rate,
          r'$R$ [bits/s/Hz]',
          'new_rate_vs_iter',
          zoom_x=(20, 42), zoom_y=(21, 24))

conv_plot(3, data_tau,
          r'$\bar{\tau}$',
          'new_beam_vs_iter',
          zoom_x=(20, 42), zoom_y=(20, 30))
...
keys = ['J20_I120', 'J20_I80', 'nc1', 'nc2', 'nc3', 'nc4']
...
for ss in range(len(snr_dB_list)):
    ...
    with torch.no_grad():
        ...
        _, _, F, W = model_UPGA_J20_I80.execute_PGA(
            H_test, R_ss, snr_ss, I_T_new, n_iter_inner_J20)
...
def plot_snr_curves(ax, data_dict, show_legend=False):
    ...
    ax.plot(snr_dB_list_np, data_dict['nc1'],       ':',
            color=COLORS['nc1'],          linewidth=2.5,
            marker='h', markersize=6, label=LABELS['nc1'])
