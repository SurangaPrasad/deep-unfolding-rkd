...
iter_number_new_teacher = np.array(list(range(I_T_new + 1)))
...
# ── Teacher J20/I60 ───────────────────────────────────────────────────────────
_ckpt_J20_I60  = torch.load('UPGA_J20_I60_w_0_30.pth', map_location='cpu',
                             weights_only=False)
_ss_shape_I60  = _ckpt_J20_I60['step_size'].shape
_ss_J20_I60    = torch.zeros(_ss_shape_I60)
model_UPGA_J20_I60 = PGA_Unfold_J20(_ss_J20_I60)
model_UPGA_J20_I60.load_state_dict(_ckpt_J20_I60)
model_UPGA_J20_I60.eval()
_actual_I_J20_I60 = _ss_shape_I60[1]
with torch.no_grad():
    sr_J20_I60, t_J20_I60, _, _ = model_UPGA_J20_I60.execute_PGA(
        H_test, R, snr, _actual_I_J20_I60, n_iter_inner_J20)
rate_iter_J20_I60 = [r.detach().numpy() for r in (sum(sr_J20_I60) / len(H_test[0]))]
tau_iter_J20_I60  = [e.detach().numpy() for e in (sum(t_J20_I60)  / len(H_test[0]))]
iter_number_new_teacher = np.array(list(range(_actual_I_J20_I60 + 1)))
print(f'  Teacher J20/I60 loaded — actual I={_actual_I_J20_I60}')

# ── J10/I120 truncated at I=30 (no KD baseline) ───────────────────────────────
print('Loading J10/I120 truncated at I=30...')
_ckpt_J10_all  = torch.load('./model/UPGA_J10_all.pth',
                             map_location='cpu', weights_only=False)
_ss_shape_J10  = _ckpt_J10_all['step_size'].shape   # [10, 120, K+1]
...
iter_number_J10_trunc = np.array(list(range(I_S_new + 1)))
iter_number_nc1 = np.array(list(range(I_S_new + 1)))
print(f'  J10/I120 truncated at I={I_S_new}')

# ── nc1 = J10/I40 (trained), truncated at I=30 ────────────────────────────────
# NOTE: adjust this path to match the actual saved checkpoint filename for
# the J10/I40 trained model.
J10_I40_PATH = './model/UPGA_J10_320.pth_IS40_IT80_cell_1_flat0.01_noRKD_Kl5_win10_030.pth'

print('Loading J10/I40 model (nc1, truncated at I=30)...')
_ckpt_J10_I40 = torch.load(J10_I40_PATH, map_location='cpu',
                            weights_only=False)
_ss_shape_J10_I40 = _ckpt_J10_I40['step_size'].shape   # expected [10, 40, K+1]
print(f'  step_size shape: {list(_ss_shape_J10_I40)}')
model_nc1 = PGA_Unfold_J10(torch.zeros(_ss_shape_J10_I40))
model_nc1.load_state_dict(_ckpt_J10_I40)
model_nc1.eval()
with torch.no_grad():
    sr_nc1, t_nc1, _, _ = model_nc1.execute_PGA(
        H_test, R, snr, I_S_new, n_iter_inner_J10)
rate_iter_nc1 = [r.detach().numpy() for r in (sum(sr_nc1) / len(H_test[0]))]
tau_iter_nc1  = [e.detach().numpy() for e in (sum(t_nc1)  / len(H_test[0]))]

# ── New student cell_2 ────────────────────────────────────────────────────────
print('Loading new cell_2...')
model_nc2 = load_new_student('./model/UPGA_J10_320.pth_IS30_IT60_cell_2_flat0.01_RKDlog_Kl15_win10_030_prop.pth')
with torch.no_grad():
...
model_nc3 = load_new_student('./model/UPGA_J10_320.pth_IS30_IT60_cell_3_AGT_avg_pairs_noRKD_Kl15_win10_030_prop.pth')
with torch.no_grad():
...
model_nc4 = load_new_student('./model/UPGA_J10_320.pth_IS30_IT60_cell_4_AGT_avg_pairs_RKDlog_Kl15_win10_030_prop.pth')
with torch.no_grad():
...
obj_iter_J20_I60   = [r - OMEGA * t for r, t in zip(rate_iter_J20_I60,   tau_iter_J20_I60)]
...
print(f'  {"-"*83}')
results_table = {
    'Teacher (J=20, I=60)'                    : (obj_iter_J20_I60[-1],   rate_iter_J20_I60[-1],   tau_iter_J20_I60[-1],   1600),
    f'J10/I120 truncated at I={I_S_new}'      : (obj_iter_J10_trunc[-1], rate_iter_J10_trunc[-1], tau_iter_J10_trunc[-1], J_S_new*n_iter_inner_J10),
    f'Student J10/I{I_S_new} (from J10/I40 ckpt)' : (obj_iter_nc1[-1],   rate_iter_nc1[-1],       tau_iter_nc1[-1],       J_S_new*I_S_new),
...
LABELS = {
    'teacher_I60'  : r'Teacher $(J=20, I=60)$',
    'nc1'    :  rf'Student $(J=10, I={I_S_new})$ ',
...
def conv_plot(fig_num, data_dict, ylabel, filename,
    ...
    ax.plot(iter_number_new_teacher,  data_dict['J20_I60'],   '--',
            color=COLORS['teacher_I60'],  linewidth=3,
            marker='o', markevery=8, markersize=6,
            label=LABELS['teacher_I60'])
    ax.plot(iter_number_nc1,    data_dict['nc1'], ':',
    ...
    ax.set_xlim(0, _actual_I_J20_I60)
    ax.grid()
    ...
    if zoom_y is not None:
        ...
        axins.plot(iter_number_new_teacher, data_dict['J20_I60'],   '--',
                   color=COLORS['teacher_I60'],  linewidth=1.5,
                   marker='o', markevery=8, markersize=3)
        axins.plot(iter_number_nc1,   data_dict['nc1'], ':',
...
keys = ['J20_I60', 'nc1', 'nc1', 'nc2', 'nc3', 'nc4']
...
for ss in range(len(snr_dB_list)):
    ...
    with torch.no_grad():
        # Teacher J20/I60 — full depth
        _, _, F, W = model_UPGA_J20_I60.execute_PGA(
            H_test, R_ss, snr_ss, _actual_I_J20_I60, n_iter_inner_J20)
        rate_vs_snr['J20_I60'].append(get_sum_rate(H_test, F, W, snr_ss).item())
        mse_vs_snr['J20_I60'].append(get_MSE(F, W, at_ss, R_ss, snr_ss).item())

        # nc1 = J10/I40 truncated at EVAL_ITERS
        _, _, F, W = model_nc1.execute_PGA(
...
def plot_snr_curves(ax, data_dict, show_legend=False):
    ax.plot(snr_dB_list_np, data_dict['J20_I60'],   '--',
            color=COLORS['teacher_I60'],  linewidth=3,
            marker='o', markersize=6, label=LABELS['teacher_I60'])
