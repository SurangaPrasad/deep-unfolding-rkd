...
Modifications from original Unified_final.py:
  1. Embedding: F_RF → F_RF @ F_BB (effective beamformer)
  2. RKD weights: linear w_l → log(l)
  3. New L_log loss: sum_l log(l) * ||J_T^(I_T) - J_S^(l)||^2 across all 60 layers
  4. Student forward pass returns rate_over_iters and tau_over_iters for L_log
  5. Teacher forward pass also returns final objective J_T^(I_T)
  6. RKD still covers only 30/60 layers (original windows):
       Early: student [0,  15) <- teacher [20, 35)
       Late:  student [45, 60) <- teacher [105, 120)

Four ablation cells:
  cell_1: flat init, no RKD, no log  (task loss only — truncated baseline)
  cell_2: flat init, + RKD + log
  cell_3: AGT  init, no RKD, no log  (task loss only)
  cell_4: AGT  init, + RKD + log     (proposed)
"""
...
# ══════════════════════════════════════════════════════════════════════════════
ABLATION_CELL = 'cell_4'   # 'cell_1' | 'cell_2' | 'cell_3' | 'cell_4'
# ══════════════════════════════════════════════════════════════════════════════
...
def build_agt_init(teacher_model,
    ...
    with torch.no_grad():
        ...
        if inner_strategy == 'avg_pairs':
            ...
            inner_desc = "subsample"

        else:
            raise ValueError(f"Unknown inner_strategy {inner_strategy!r}")

        # ---------- Outer compression ----------
        if outer_strategy == 'avg_pairs':
            assert n_outer_t == 2 * n_outer_S

            # [Js,120,K+1] -> [Js,60,2,K+1]
            ss_init = (
                ss_inner
                .view(N_INNER_S, n_outer_S, 2, K + 1)
                .mean(dim=2)
            )
            outer_desc = "avg_pairs"

        elif outer_strategy == 'first':
        ...
        elif outer_strategy == 'subsample':
            indices = torch.linspace(0, n_outer_t - 1, n_outer_S).long()
            ss_init = ss_inner[:, indices].clone()
            outer_desc = "subsample"
    ...
    print(f"  {list(ss_T.shape)} -> {list(ss_inner.shape)} -> {list(ss_init.shape)}")
...
def get_effective_beamformer(F, W, B):
    """
    Compute effective beamformer FW and flatten to real vector.
    F: [K, B, Nt, Nrf]  W: [K, B, Nrf, M]
    FW: [K, B, Nt, M]
    phi: [B, 2*K*Nt*M]  (real vector embedding)
    """
    # Matrix multiply F @ W for each freq band and batch sample
    FW = torch.matmul(F, W)                          # [K, B, Nt, M]
    FW = FW.permute(1, 0, 2, 3).reshape(B, -1)       # [B, K*Nt*M] complex
    # Convert complex to real by stacking Re and Im
...
# ══════════════════════════════════════════════════════════════════════════════
# 3.  UNIFIED LOSS: L_log + CI-RKD with log(l) weights
# ══════════════════════════════════════════════════════════════════════════════
...
def log_weighted_rkd_loss(F_T_window, W_T_window,
    ...
    """
    CI-RKD loss on effective beamformer FW with log(l) weighting.

    For each layer l in [1, K_layers]:
      w_l = log(l)  -- grows with l, giving more weight to
                       later more-converged iterates

    L_CI = sum_l log(l) * [lambda_dist * L_dist^(l)
                          + lambda_angle * L_angle^(l)]

    Uses effective beamformer phi^(l) = vec_R(F^(l) @ W^(l))
    instead of just F^(l), capturing combined analog+digital effect.
    """
    # Build log weights: log(1)=0, log(2), ..., log(K_layers)
    # l indexes from 1 to K_layers
    log_w = torch.log(
        torch.arange(1, K_layers + 1, dtype=torch.float32,
                     device=device).clamp(min=1e-8))   # [K_layers]

    total_dist  = 0.0
    total_angle = 0.0

    for l in range(K_layers):
        w_l = log_w[l].item()   # scalar weight for this layer

        # Compute effective beamformer embeddings for teacher and student
        phi_T = get_effective_beamformer(
            F_T_window[l].detach(), W_T_window[l].detach(), B)  # [B, D]
        phi_S = get_effective_beamformer(
            F_S_window[l], W_S_window[l], B)                     # [B, D]

        # ── Distance loss ──────────────────────────────────────────────────
        with torch.no_grad():
            d_T  = torch.cdist(phi_T.unsqueeze(0),
                               phi_T.unsqueeze(0), p=2).squeeze(0)  # [B,B]
            mu_T = d_T.mean().clamp(min=1e-12)
            d_T  = d_T / mu_T

        d_S  = torch.cdist(phi_S.unsqueeze(0),
                           phi_S.unsqueeze(0), p=2).squeeze(0)      # [B,B]
        mu_S = d_S.mean().clamp(min=1e-12)
        d_S  = d_S / mu_S

        dist_l = nn.functional.smooth_l1_loss(d_S, d_T)
        total_dist += w_l * dist_l

        # ── Angle loss ────────────────────────────────────────────────────
        with torch.no_grad():
            e_T   = phi_T.unsqueeze(0) - phi_T.unsqueeze(1)   # [B,B,D]
            e_T   = nn.functional.normalize(e_T, p=2, dim=-1)
            cos_T = torch.bmm(e_T, e_T.permute(0, 2, 1))      # [B,B,B]

        e_S   = phi_S.unsqueeze(0) - phi_S.unsqueeze(1)
        e_S   = nn.functional.normalize(e_S, p=2, dim=-1)
        cos_S = torch.bmm(e_S, e_S.permute(0, 2, 1))

        angle_l = nn.functional.smooth_l1_loss(cos_S, cos_T)
        total_angle += w_l * angle_l

    rkd_loss = lambda_dist * total_dist + lambda_angle * total_angle
    return rkd_loss
...
def log_weighted_deep_loss(rate_over_iters, tau_over_iters,
    ...
    """
    Log-weighted deep supervision loss (professor's suggestion):

      L_log = sum_{l=1}^{I_S} log(l) * ||J_T^(I_T) - J_S^(l)||^2

    where:
      J_T^(I_T) = teacher's final converged objective (fixed scalar)
      J_S^(l)   = student's objective at outer layer l
      log(l)    = weight growing with l (early layers less reliable)

    rate_over_iters: [I_S, B]
    tau_over_iters:  [I_S, B]
    J_T_final:       scalar — teacher's J at final layer
    """
    device_  = rate_over_iters.device
    omega_t  = torch.as_tensor(OMEGA, device=device_, dtype=torch.float32)

    # Student objective at every outer layer: [I_S, B]
    J_S = rate_over_iters - omega_t * tau_over_iters

    # Log weights: log(1)=0, log(2), ..., log(I_S)  shape [I_S, 1]
    log_w = torch.log(
        torch.arange(1, I_S + 1, dtype=torch.float32,
                     device=device_).clamp(min=1e-8)).unsqueeze(1)

    # Squared difference from fixed teacher target, averaged over batch
    sq_diff = (J_S - J_T_final.detach()).pow(2)   # [I_S, B]
    loss    = (log_w * sq_diff.mean(dim=1)).sum()  # scalar
...
class PGA_Unfold_J20_Teacher(PGA_Unfold_J20):
    """
    Teacher wrapper collecting two trajectory windows.
    ORIGINAL two-window design (30/60 layers):
      Early window: teacher [t_e,       t_e + K_w)   -- steepest descent
      Late  window: teacher [I_T - K_w, I_T)          -- converged plateau
    Also returns final objective J_T^(I_T) for L_log.
    All tensors detached — teacher is frozen.
    """
    ...
    def execute_PGA_two_windows(self, H, R, Pt,
        ...
        # Collect F and W at each window iteration (both needed for FW embedding)
        ...
        for ii in range(n_iter_outer):
            ...
            # Collect window iterates (F and W both needed for FW embedding)
        ...
        # Teacher final objective J_T^(I_T) for L_log
        ...
        J_T_final = (R_final - omega_t * tau_final).detach()   # scalar
...
class PGA_Unfold_J10_I60_v2(nn.Module):
    """
    Student — 10 inner x 60 outer.
    Returns rate_over_iters and tau_over_iters for L_log,
    and F,W trajectories at both windows for CI-RKD.
    """
    ...
    def execute_PGA_with_windows(self, H, R, Pt,
        ...
        Forward pass collecting:
          - rate_over_iters, tau_over_iters: [I_S, B] for L_log
          - F_traj_first, W_traj_first:  early window [0, K_layers)
          - F_traj_last,  W_traj_last:   late window [I_S-K_layers, I_S)
          - Final F, W for task loss

        collect_windows=False for task-only cells (no RKD)
        """
        ...
        # Window trajectory storage (F and W both needed for FW embedding)
        ...
        for ii in range(n_iter_outer):
            ...
            # Collect window iterates for RKD
            if collect_windows:
                if ii < K_layers:
            ...
            # Always collect per-layer objective for L_log
...
if run_RKD_Distillation == 1:
    ...
    # RKD window parameters (ORIGINAL 30-layer design)
    K_layers      = 15             # each window = 15 layers
    t_start_early = 20             # teacher early window: [20, 35)
    t_start_late  = I_T - K_layers # teacher late  window: [105, 120)
    # student early window: [0,  15)
    # student late  window: [45, 60)

    # Loss weights
    lambda_dist   = 25.0    # RKD distance loss weight
    lambda_angle  = 50.0    # RKD angle loss weight
    lambda_log    = 0.00001    # log-weighted deep supervision weight
    lambda_late   = 1.0     # late window RKD relative weight
    ...
    assert K_layers <= I_S // 2, \
        f"Windows overlap — reduce K_layers to <= {I_S // 2}"
    ...
    print(f'Teacher loaded: {'UPGA_J20_I120_w_030.pth'}\n')

    # ── Build step-size initialization ────────────────────────────────────────
    if USE_AGT:
    ...
    rkd_tag = 'RKDlog_30layers' if USE_RKD else 'noRKD'
    ...
    for i_epoch in range(n_epoch):
        ...
        for i_batch in range(0, len(H_train[0]), batch_size):
            ...
            # ── Teacher pass (needed for both RKD windows and J_T_final) ──────
            if USE_RKD:
                with torch.no_grad():
                    (_, _,
                     F_T_early, W_T_early,
                    ...
                    epoch_task_t += get_sum_loss(
                        F_T_early[-1], W_T_late[-1], H, R, snr_train, B).item()
            ...
            # ── Task loss: terminal output only ────────────────────────────────
            loss_task  = get_sum_loss(F_s, W_s, H, R, snr_train, B)
            total_loss = loss_task

            if USE_RKD:
                # ── L_log: all 60 student layers toward teacher final J ────────
                # J_T_final: teacher's best converged objective (fixed target)
                # rate/tau_over_iters: student objective at every outer layer
                loss_log = log_weighted_deep_loss(
                    rate_over_iters, tau_over_iters,
                    J_T_final, lambda_log=lambda_log)

                # ── CI-RKD early window: student [0,15) <- teacher [20,35) ─────
                # Uses FW embedding, log(l) weights
                ...
                # ── CI-RKD late window: student [45,60) <- teacher [105,120) ───
