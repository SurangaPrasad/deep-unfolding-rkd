"""
Ablation 3: J5/I60 — NO INIT, INNER RKD ONLY
=============================================
Random step_size initialisation.
Only the inner-layer RKD loss supervises training.
No outer FW RKD. No AGT initialisation.

Isolates the contribution of inner RKD alone from random init.
Compare against ablation1 to see inner RKD gain.
Compare against code1 to separate RKD vs init contributions.
Compare against code2 to see what init + outer FW adds on top.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# INNER RKD LOSS  (identical to document 14 — unchanged)
# ══════════════════════════════════════════════════════════════════════════════
...
if run_RKD_Distillation == 1:
    ...
    print('=' * 70)
    print('ABLATION 3: NO INIT, INNER RKD ONLY  —  J5/I60')
    ...
    for i_epoch in range(n_epoch):
        ...
        print(f"Epoch {i_epoch:4d} [inner_rkd_only] | "
        ...
        if current_student > best_student_loss:
            ...
            torch.save(model_student.state_dict(),
                       ...
                       '_J3_I60_INNER_RKD_ONLY_best.pth')
            print(f"  [BEST] epoch {i_epoch} → saved")

    torch.save(model_student.state_dict(),
               model_file_name_UPGA_J10 + '_J3_I60_INNER_RKD_ONLY.pth')
    print(f'\nSaved.')
