"""
Ablation 2: J5/I60 — NO INIT, OUTER FW RKD ONLY
=================================================
Random step_size initialisation.
Only the outer FW RKD loss (on effective beamformer FW) supervises training.
No inner-layer RKD. No AGT initialisation.

Isolates the contribution of outer FW RKD alone.
Compare against ablation1 to see FW RKD gain from random init.
Compare against code2 to see what inner RKD + init adds on top.
"""
...
def rkd_angle_loss(teacher_repr, student_repr):
    with torch.no_grad():
        t_e   = teacher_repr.unsqueeze(0) - teacher_repr.unsqueeze(1)
...
# ══════════════════════════════════════════════════════════════════════════════
# TEACHER MODEL  (standard execute_PGA — no inner collection needed)
# ══════════════════════════════════════════════════════════════════════════════

class PGA_Unfold_J20_Teacher(PGA_Unfold_J20):
    pass   # uses inherited execute_PGA unchanged
...
if run_RKD_Distillation == 1:
    ...
    print('=' * 70)
    print('ABLATION 2: NO INIT, OUTER FW RKD ONLY  —  J5/I60')
    ...
    # Random initialisation — no AGT
    model_student = PGA_Unfold_J3_I60_CI(step_size_UPGA_J3_I60).to(device)
    ...
    for i_epoch in range(n_epoch):
        ...
        for i_batch in range(0, len(H_train[0]), batch_size):
            ...
            student_repr = nn.functional.normalize(FW_s_flat, dim=1)

            loss_task   = get_sum_loss(F_s, W_s, H, R, snr_train, B)
            loss_rkd_fw = (lambda_dist_fw * rkd_distance_loss(
                               teacher_repr, student_repr) +
                           lambda_angle_fw * rkd_angle_loss(
                               teacher_repr, student_repr))

            total_loss = loss_task + lambda_fw * loss_rkd_fw
        ...
        print(f"Epoch {i_epoch:4d} [outer_fw_only] | "
        ...
        if current_student > best_student_loss:
            ...
            torch.save(model_student.state_dict(),
                       ...
                       '_J3_I60_OUTER_FW_ONLY_best.pth')
            print(f"  [BEST] epoch {i_epoch} → saved")

    torch.save(model_student.state_dict(),
               model_file_name_UPGA_J10 + '_J3_I60_OUTER_FW_ONLY.pth')
    print(f'\nSaved.')
