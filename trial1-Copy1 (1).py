...
if run_RKD_Distillation == 1:
    ...
    print('=' * 70)
    print(' J10/I60 student  ←  J20/I120 teacher  [INNER-ONLY INIT]')
    ...
    print(f'Student step_size shape : {list(model_student.step_size.shape)}\n')
        ...
        # ── Baseline init: uniform step sizes, no teacher ────────────────────────
    #step_size_seed = 0.01   # tune this; ~0.01–0.05 is typical for PGA
    
    #ss_init = torch.full((N_INNER_S, I_S, K + 1), step_size_seed)
    #print(f"\n[J10/I60 baseline init | torch.full({step_size_seed})]")
    #print(f"  step_size shape : {list(ss_init.shape)}")
    #print(f"  all values      : {step_size_seed}\n")
    
    # ── Instantiate student ───────────────────────────────────────────────────
    #model_student = PGA_Unfold_J10_I60_CI(ss_init).to(device)
    #optimizer     = torch.optim.Adam(model_student.parameters(), lr=learning_rate) #change here for no initial
    ...
    save_path = (model_file_name_UPGA_J10 +
                 f'_I60_no_init_sym_inner_{inner_strategy}'
                 f'_Kl{K_layers}_win{t_start_early}agt_new15epoch.pth')
